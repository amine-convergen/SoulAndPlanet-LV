...
function somfrp_get_template_html( $template_name ) {
	...
	$form_title = ( isset( $gen_options['somfrp_reset_form_title'] ) && $gen_options['somfrp_reset_form_title'] )
		...
		: esc_html__( 'Réinitialiser le mot de passe', 'frontend-reset-password' ) ;
	...
	if ( empty( $lost_text ) ) {
		$lost_text = esc_html__( "Veuillez saisir votre adresse e-mail ou votre nom d'utilisateur. Vous recevrez un lien pour créer un nouveau mot de passe par e-mail.", 'frontend-reset-password' );
	...
	if ( empty( $min_length ) ) {
		...
		if ( empty( $reset_text ) ) {
			$reset_text = esc_html__( 'Veuillez entrer un nouveau mot de passe.', 'frontend-reset-password' );
		...
		if ( empty( $reset_text ) ) {
			$reset_text = sprintf( esc_html__( 'Veuillez saisir un nouveau mot de passe. Minimum %s caractères.', 'frontend-reset-password' ), $min_length );
			$reset_text_output = '<p class="extra-space">' . $reset_text . '</p>';
		} else {
			$allowed_tags = somfrp_get_allowed_html_tags();
			$reset_text_output = wpautop( wp_kses( $reset_text, $allowed_tags ) );
	...
	if ( ! $email_confirmed && isset( $_GET['somresetpass'] ) && ( isset( $_GET['somfrp_action'] ) && $_GET['somfrp_action'] == 'rp' ) ) {
		...
		if ( is_wp_error( $user ) ) {
			...
			if ( $user->get_error_code() === 'expired_key' ) {
				...
				$errors['expired_key'] = esc_html__( 'Cette clé a expiré. Veuillez réinitialiser à nouveau votre mot de passe.', 'frontend-reset-password' );
				...
				$errors['invalid_key'] = esc_html__( "Cette clé n'est plus valide. Veuillez réinitialiser à nouveau votre mot de passe. Code: " . $code, 'frontend-reset-password' );
...
function somfrp_lost_pass_handler( $action = '' ) {
	...
	if ( ! somfrp_verify_nonce_request( 'somfrp_lost_pass' ) ) {
		somfrp_wp_error( '<strong>' . esc_html__('ERROR', 'frontend-reset-password') . ':</strong> ' . esc_html__( "quelque chose s'est mal passé avec ça!", 'frontend-reset-password' ) );
	}
	...
	if ( isset( $user_info ) && ! empty( $user_info ) ) {
		...
		if ( strpos( $user_info, '@' ) ) {
			...
			if ( empty( $user_data ) ) {
				$errors['no_email'] = esc_html__( "Cette adresse e-mail n'est pas reconnue.", 'frontend-reset-password' );
			}
		} else {
			$user_data = get_user_by( 'login', $user_info ); 
			if ( empty( $user_data ) ) {
				$errors['no_login'] = esc_html__( "Ce nom d'utilisateur n'est pas reconnu.", 'frontend-reset-password' );
			}
		}

	} else {

		$errors['invalid_input'] = esc_html__( "Veuillez saisir un nom d'utilisateur ou une adresse e-mail.", 'frontend-reset-password' );
...
function somfrp_lost_pass_callback() {
	...
	if ( empty( $user_info ) ) {
		...
		somfrp_wp_error( '<strong>' . esc_html__('ERROR', 'frontend-reset-password') . ':</strong> ' . esc_html__( "Veuillez ajouter votre adresse e-mail ou votre nom d'utilisateur !", 'frontend-reset-password' ) );
		exit;
		...
		$user_data = get_user_by( 'email', $user_info );
		if ( empty( $user_data ) ) {
			somfrp_wp_error( '<strong>' . esc_html__('ERROR', 'frontend-reset-password') . ':</strong> ' . esc_html__( "Aucune adresse e-mail trouvée !", 'frontend-reset-password' ) );
			exit;
		}
	...
	if ( is_wp_error( $key ) ) {
		somfrp_wp_error( '<strong>' . esc_html__('ERROR', 'frontend-reset-password') . ':</strong> ' . esc_html__( 'clé introuvable!', 'frontend-reset-password' ) );
		//return $key;
	}
	...
	if ( empty( $email_body ) ) {
		...
		<p><?php _e( "Quelqu'un a demandé que le mot de passe soit réinitialisé pour le compte suivant:", 'frontend-reset-password' ); ?></p>
		<p><?php printf( esc_html__( 'Username: %s', 'frontend-reset-password' ), $user_login ); ?></p>
		<p><?php _e( "S'il s'agit d'une erreur, ignorez cet e-mail et rien ne se passera.", 'frontend-reset-password' ); ?></p>
		<p><?php _e( "Pour réinitialiser votre mot de passe, rendez-vous à l'adresse suivante :", 'frontend-reset-password' ); ?></p>
	...
	: esc_html__( 'Réinitialisation du mot de passe du compte', 'frontend-reset-password' );
	...
	if ( wp_mail( $user_email, wp_specialchars_decode( $title ), $message, $headers ) ) {
		...
		$errors['password_email_not_sent'] = esc_html__( "L'e-mail n'a pas pu être envoyé.", 'frontend-reset-password' );
...
function somfrp_reset_pass_handler( $action = '' ) {
	...
	if ( empty( $user_pass ) || empty( $user_pass_repeat ) ) {
		$errors['no_password'] = esc_html__( 'Veuillez entrer un nouveau mot de passe.', 'frontend-reset-password' );
		$_REQUEST['errors'] = $errors;
		return;
	} elseif ( $user_pass !== $user_pass_repeat ) {
		$errors['password_mismatch'] = esc_html__( 'Les mots de passe ne correspondent pas.', 'frontend-reset-password' );
		$_REQUEST['errors'] = $errors;
		return;
	}
	...
	if ( empty( $key ) || empty( $user_id ) ) {
		$errors['key_login'] = esc_html__( "Le lien de réinitialisation n'est pas valide.", 'frontend-reset-password' );
		$_REQUEST['errors'] = $errors;
	...
	if ( is_wp_error( $user ) ) {
		...
		if ( $user->get_error_code() === 'expired_key' ) {
			...
			$errors['expired_key'] = esc_html__( 'Désolé, cette clé a expiré. Veuillez réinitialiser à nouveau votre mot de passe.', 'frontend-reset-password' );

		} else {

			$errors['invalid_key'] = esc_html__( 'Désolé, cette clé ne semble pas valide. Veuillez réinitialiser à nouveau votre mot de passe.', 'frontend-reset-password' );
