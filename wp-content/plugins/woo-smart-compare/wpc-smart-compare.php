...
! defined( 'WOOSC_VERSION' ) && define( 'WOOSC_VERSION', '4.2.3' );
		...
		if ( ! class_exists( 'WPCleverWoosc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosc {
                                ...
                                <form method="post" action="options.php">
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
												<?php $woosc_bar_btn_color_default = apply_filters( 'woosc_bar_btn_color_default', '#00a0d2' ); ?>
					...
					if ( is_array( $woosc_products ) && ( count( $woosc_products ) > 0 ) ) {
							...
							foreach ( $saved_fields as $saved_field ) {
								switch ( $saved_field ) {
									...
									case 'availability':
										...
										break;
									default:
										$woosc_products_data[ $woosc_product ][ $saved_field ] = apply_filters( 'woosc_product_' . $saved_field, '', $product );
								}
							}
						}
						...
						foreach ( $saved_fields as $saved_field ) {
							...
							if ( ! in_array( $saved_field, array(
								...
								$woosc_field .= '<tr class="tr-default tr-' . esc_attr( $saved_field ) . ' ' . ( ! in_array( $saved_field, $cookie_fields ) ? 'tr-hide' : '' ) . '"><td class="td-label">' . esc_html( self::$woosc_fields[ $saved_field ] ) . '</td>';
				...
				function woosc_search() {
					if ( apply_filters( 'woosc_verify_nonce', true ) && ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'woosc-nonce' ) ) ) {
				...
				function woosc_shortcode( $atts ) {
					...
					if ( $atts['id'] ) {
						...
						if ( ! empty( $cats ) && ( $cats[0] !== '0' ) ) {
							if ( ! has_term( $cats, 'product_cat', $atts['id'] ) ) {
								return '';
							}
						}
