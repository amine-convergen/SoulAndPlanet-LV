...
class WPIE_Product_Attributes_Export extends \wpie\export\engine\WPIE_Export_Engine {
        ...
        protected function get_fields() {
                ...
                $attribute_fields = array(
                        'title' => __( "Attribute", 'vj-wp-import-export' ),
                        "isDefault" => true,
                        "isFiltered" => false,
                        'data' => array(
                                array(
                                        'name' => 'Attribute Name',
                                        'type' => 'attribute_label',
                                        'isDefault' => true
                                ),
                                array(
                                        'name' => 'Attribute Slug',
                                ...
                                array(
                                        'name' => 'Attribute Order by',
                                        'type' => 'attribute_orderby',
                                        'isDefault' => true
                                ),
                                array(
                                        'name' => 'Attribute Enable archives?',
                                        'type' => 'attribute_public',
                                        'isDefault' => true
                                )
                        )
                );
                ...
                $export_fields = array(
                        ...
                        "attribute" => apply_filters( 'wpie_taxonomy_attribute_fields', $attribute_fields ),
                ...
                $export_fields[ 'meta' ] = array(
                        "title" => __( "Custom Fields", 'vj-wp-import-export' ),
                        "data" => $meta_data
                );

                unset( $standard_fields, $attribute_fields, $addon_class, $meta_data );
        ...
        protected function process_export() {
                ...
                if ( isset( $this->template_options[ 'wpie_attribute_taxonomy' ] ) && !empty( $this->template_options[ 'wpie_attribute_taxonomy' ] ) ) {
                        $taxonomy = $this->template_options[ 'wpie_attribute_taxonomy' ];
                } else {
                        $taxonomy = $this->get_attribute_taxonomies();
                }

                $taxonomy = $this->validate_taxonomy_slug( $taxonomy );
        ...
        protected function process_items( $taxonomy_data = [] ) {
                ...
                if ( !empty( $taxonomy_data ) ) {
                        ...
                        foreach ( $taxonomy_data as $term_taxonomy_id ) {

                                $wpie_export_id = $term_taxonomy_id;

                                $item = get_term_by( "term_taxonomy_id", $term_taxonomy_id );

                                if ( $item && !empty( $fields_data ) ) {

                                        $taxonomy = isset( $item->taxonomy ) ? $item->taxonomy : "";

                                        $attribute = $this->get_attribute_by_taxonomy( $taxonomy );
                                        ...
                                        foreach ( $fields_data as $field ) {
                                                ...
                                                switch ( $field_type ) {
                                                        ...
                                                        case 'attribute_public':
                                                                $attr_field = isset( $attribute->$field_type ) ? $attribute->$field_type : "";
                                                                $this->export_data[ $field_name ] = apply_filters( 'wpie_export_' . $field_type, $this->apply_user_function( $attr_field, $is_php, $php_func ), $item );
                                                                break;
                                                        case 'term_id':
        ...
        private function validate_taxonomy_slug( $slug ) {

                if ( empty( $slug ) ) {
                        return "";
                }
                if ( is_array( $slug ) ) {
                        return array_map( [ $this, "validate_taxonomy_slug" ], $slug );
                }
                return \wc_attribute_taxonomy_name( preg_replace( '/^pa\_/', '', $slug ) );
        }

        private function get_attribute_by_taxonomy( $taxonomy = "" ) {
                ...
                $slug = preg_replace( '/^pa\_/', '', $taxonomy );

                $attribute = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name = %s ORDER BY attribute_name ASC;", $slug ) );

                if ( empty( $attribute ) ) {
                        return;
                }

                $this->attributes[ $taxonomy ] = $attribute;
        ...
        private function get_attribute_taxonomies() {
                ...
                global $wpdb;
                return $wpdb->get_col( "SELECT attribute_name FROM {$wpdb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name != ''  ORDER BY attribute_name ASC;" );
        }
