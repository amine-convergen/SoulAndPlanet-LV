...
class WPIE_Export {
        ...
        public function init_export( $export_type = "post", $opration = "export", $template = null ) {
                ...
                if ( class_exists( $export_engine ) ) {
                        ...
                        return new \WP_Error( 'wpie_import_error', sprintf( __( 'Class %s Not Exist', 'vj-wp-import-export' ), $export_engine ) );
                }
        ...
        private function get_deafult_export_type() {
                ...
                return [
                        "post" => __( 'Post', 'vj-wp-import-export' ),
                        "page" => __( 'Page', 'vj-wp-import-export' ),
                        "product" => __( 'WooCommerce Products', 'vj-wp-import-export' ),
                        "taxonomies" => __( 'Taxonomies | Categories | Tags', 'vj-wp-import-export' ),
                        "users" => __( 'Users', 'vj-wp-import-export' ),
                        "comments" => __( 'Comments', 'vj-wp-import-export' ),
                        "product_reviews" => __( 'Product Reviews', 'vj-wp-import-export' ),
                        "product_attributes" => __( 'Product Attributes', 'vj-wp-import-export' ),
                        "shop_order" => __( 'WooCommerce Orders', 'vj-wp-import-export' ),
                        "shop_coupon" => __( 'WooCommerce Coupons', 'vj-wp-import-export' ),
                        "shop_customer" => __( 'WooCommerce Customers', 'vj-wp-import-export' ),
                ];
        }
        ...
        public function wpie_get_taxonomies() {
                ...
                $data = [
                        "category" => __( 'Post Categories', 'vj-wp-import-export' ),
                        "product_cat" => __( 'Product Categories', 'vj-wp-import-export' ),
                        "post_tag" => __( 'Post Tags', 'vj-wp-import-export' ),
                        "product_tag" => __( 'Product Tags', 'vj-wp-import-export' ),
                ];
        ...
        protected function get_export_rule() {
                ...
                $wpie_export_rules = array(
                        'wpie_tax' => array(
                                'in' => __( 'In', 'vj-wp-import-export' ),
                                'not_in' => __( 'Not In', 'vj-wp-import-export' )
                        ),
                        'wpie_date' => array(
                                'equals' => __( 'equals', 'vj-wp-import-export' ),
                                'not_equals' => __( "doesn't equal", 'vj-wp-import-export' ),
                                'greater' => __( 'newer than', 'vj-wp-import-export' ),
                                'equals_or_greater' => __( 'equal to or newer than', 'vj-wp-import-export' ),
                                'less' => __( 'older than', 'vj-wp-import-export' ),
                                'equals_or_less' => __( 'equal to or older than', 'vj-wp-import-export' ),
                                'contains' => __( 'contains', 'vj-wp-import-export' ),
                                'not_contains' => __( "doesn't contain", 'vj-wp-import-export' ),
                                'is_empty' => __( 'is empty', 'vj-wp-import-export' ),
                                'is_not_empty' => __( 'is not empty', 'vj-wp-import-export' ),
                        ),
                        ...
                        'wpie_term_parent_slug' => array(
                                'equals' => __( 'equals', 'vj-wp-import-export' ),
                                'not_equals' => __( "doesn't equal", 'vj-wp-import-export' ),
                                'greater' => __( 'greater than', 'vj-wp-import-export' ),
                                'equals_or_greater' => __( 'equal to or greater than', 'vj-wp-import-export' ),
                                'less' => __( 'less than', 'vj-wp-import-export' ),
                                'equals_or_less' => __( 'equal to or less than', 'vj-wp-import-export' ),
                                'is_empty' => __( 'is empty', 'vj-wp-import-export' ),
                                'is_not_empty' => __( 'is not empty', 'vj-wp-import-export' ),
                        ),
                        'default' => array(
        ...
        protected function save_template_data() {
                ...
                if ( $template_id > 0 ) {
                        ...
                        if ( !is_null( $options ) ) {
                                ...
                                $return_value[ 'message' ] = __( 'Setting updated successfully', 'vj-wp-import-export' );
                ...
                if ( $is_exist === false ) {
                        ...
                        if ( $template_id && absint( $template_id ) > 0 ) {
                                ...
                                $return_value[ 'message' ] = __( 'Setting Saved Successfully', 'vj-wp-import-export' );
                        } else {

                                $return_value[ 'status' ] = 'error';

                                $return_value[ 'message' ] = __( 'Fail to save Setting in database', 'vj-wp-import-export' );
                        }
                        unset( $template_id );
                } else {
                        $return_value[ 'status' ] = 'error';

                        $return_value[ 'message' ] = __( 'Setting Name Already Exists', 'vj-wp-import-export' );
                }
        ...
        protected function update_process_status() {
                ...
                if ( $wpie_import_id > 0 ) {
                        ...
                        if ( $process_status == "bg" ) {
                                ...
                                $return_value[ 'message' ] = __( 'Background Process Successfully Set', 'vj-wp-import-export' );
                        } elseif ( $process_status == "stop" ) {

                                $new_satus = "stopped";

                                $return_value[ 'message' ] = __( 'Process Stopped Successfully', 'vj-wp-import-export' );
                        }
                        ...
                        if ( $new_satus != "" ) {
                                ...
                                $return_value[ 'message' ] = __( 'Empty Status', 'vj-wp-import-export' );
                        }

                        unset( $new_satus );
                } else {
                        $return_value[ 'message' ] = __( 'Template id not found', 'vj-wp-import-export' );
                }
        ...
        protected function process_export_file( $export_id = "" ) {
                ...
                if ( $export_id > 0 ) {
                        ...
                        if ( $template !== false ) {
                                ...
                                return new \WP_Error( 'woo_import_export_error', __( 'Template not found', 'vj-wp-import-export' ) );
                        }
                        unset( $template );
                } else {
                        return new \WP_Error( 'woo_import_export_error', __( 'Template not found', 'vj-wp-import-export' ) );
        ...
        protected function create_zip( $options = array() ) {
                ...
                if ( $zip->open( $zipfile, \ZIPARCHIVE::CREATE ) != TRUE ) {
                        ...
                        return new \WP_Error( 'woo_import_export_error', __( 'Could not open archive', 'vj-wp-import-export' ) );
                }
