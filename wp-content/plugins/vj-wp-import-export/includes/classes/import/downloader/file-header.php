...
class File_Header {
        ...
        private function get_headers() {
                ...
                if ( $status_code !== 200 ) {
                        ...
                        if ( empty( $error ) ) {
                                $error = sprintf( __( "File Download Error : %s invalid http response status code", 'vj-wp-import-export' ), $status_code );
                        }
        ...
        private function generate_filename( $headers = [] ) {
                ...
                if ( trim( $url_ext ) !== "" ) {
                        ...
                        if ( trim( $temp_ext ) !== "" ) {
                                ...
                                $_filename = isset( $path_info[ 'filename' ] ) && !empty( $path_info[ 'filename' ] ) ? preg_replace( "/[^a-z0-9\_\-\.]/i", '', preg_replace( '#[ -]+#', '-', $path_info[ 'filename' ] ) ) : "";
                ...
                $valid_filename = empty( trim( $valid_filename ) ) ? "" : preg_replace( "/[^a-z0-9\_\-\.]/i", '', preg_replace( '#[ -]+#', '-', $valid_filename ) );
                ...
                if ( !$this->is_valid_file( $valid_filename ) ) {
                        return new \WP_Error( 'wpie_error', __( "File Download Error : Invalid File Format", 'vj-wp-import-export' ) );
                }
        ...
        private function is_valid_file( $filename = "" ) {
                ...
                if ( $this->type === "file" && !preg_match( '%\W(xml|zip|csv|xls|xlsx|xml|ods|txt|json)$%i', trim( $filename ) ) ) {
                        ...
                        return false;
                } elseif ( $this->type === "image" && !preg_match( '%\W(jpe|jpeg|jpg|gif|png|bmp|tiff|ico|svg)$%i', trim( $filename ) ) ) {
