...
class Manager
{
        ...
        public function get_filename( $url = "", $type = "file" )
        {
                ...
                if( empty( $url ) )
                {
                        return new \WP_Error( 'wpie_import_error', __( 'File Download Error : File URL is empty', 'vj-wp-import-export' ) );
                }

                if( !preg_match( '%^(http|ftp)s?://%i', $url ) )
                {
                        return new \WP_Error( 'wpie_import_error', sprintf( __( 'File Download Error : URL `%s` is not valid.', 'vj-wp-import-export' ), $url ) );
                }
        ...
        public function download( $url = "", $type = "file" )
        {
                ...
                if( !is_readable( $file ) )
                {
                        $error = new \WP_Error( 'wpie_import_error', __( 'File Download Error : File is not readable', 'vj-wp-import-export' ) );
                }
                elseif( filesize( $file ) === false || filesize( $file ) < 1 )
                {
                        $error = new \WP_Error( 'invalid_image', __( 'File Download Error : Empty File', 'vj-wp-import-export' ) );
                }
                elseif( !rename( $file, $new_file ) )
                {
                        $error = new \WP_Error( 'wpie_import_error', __( 'File Download Error : Something Wrong, File rename not work.', 'vj-wp-import-export' ) );
                }
