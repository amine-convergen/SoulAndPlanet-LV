...
class WPIE_Import_Backup {
    ...
    private function set_dir($backup_dir = "") {
        ...
        if ($backup_dir && wp_is_writable($backup_dir)) {
            ...
            return new \WP_Error('wpie_import_error', __('Backup Directory is not writable', 'vj-wp-import-export'));
        }
        return true;
    }
    ...
    public function create_backup($item_id = 0, $is_new_item = false) {
        ...
        if (!isset($this->backup[$item_id])) {
            ...
            if ($is_success === false) {
                return new \WP_Error('wpie_import_error', __('Fail To Generate Log', 'vj-wp-import-export'));
            }
            unset($is_success);
