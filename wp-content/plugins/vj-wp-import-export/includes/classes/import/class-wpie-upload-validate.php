...
class WPIE_Upload_Validate {
        ...
        public function wpie_parse_upload_data( $template_data = null, $wpie_csv_delimiter = ",", $is_first_row_title = 1, $activeFile = false, $wpie_import_id = false, $activeSheet = false, $fileFormat = false ) {
                ...
                if ( !file_exists( $file ) ) {
                        ...
                        return new \WP_Error( 'wpie_import_error', __( 'File is Empty. Please Choose another file', 'vj-wp-import-export' ) );
                }
                ...
                if ( preg_match( '%\W(txt)$%i', trim( $file_name ) ) ) {
                        ...
                        if ( copy( $file, $dest ) === false ) {
                                return new \WP_Error( 'wpie_import_error', __( 'Fail to copy file', 'vj-wp-import-export' ) );
                        }
                ...
                return new \WP_Error( 'wpie_import_error', __( 'Invalid File to parse. Please Choose other FIle', 'vj-wp-import-export' ) );
        }
        ...
        private function validateFileData( $file, $format = "xml" ) {
                ...
                if ( empty( $data ) || mb_substr( trim( $data ), 0, 1 ) !== $firstChar ) {
                        unset( $data, $fp );
                        return false;
                }
