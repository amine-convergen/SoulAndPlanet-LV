...
abstract class WPIE_Import_Engine extends \wpie\import\base\WPIE_Import_Base {
        ...
        private function init_import_process() {
                ...
                $this->set_log( "<strong>" . __( 'Record', 'vj-wp-import-export' ) . "</strong>" . " #" . ( $this->process_log[ 'imported' ] + 1) );
                ...
                if ( isset( $this->process_log[ 'last_records_status' ] ) && $this->process_log[ 'last_records_status' ] == 'pending' && isset( $this->process_log[ 'last_records_id' ] ) ) {
                        ...
                        if ( $_post ) {
                                ...
                                $this->set_log( __( 'Complete Pending Last Records', 'vj-wp-import-export' ) . " #" . $this->existing_item_id );
                        }
                ...
                if ( absint( $this->existing_item_id ) > 0 ) {
                        ...
                        $this->set_log( __( 'Existing item found', 'vj-wp-import-export' ) . " #" . $this->existing_item_id );
                }
                ...
                if ( !$this->is_new_item && $handle_items == "new" ) {
                        ...
                        $this->set_log( "<strong>" . __( 'SKIPPED', 'vj-wp-import-export' ) . '</strong> : ' . __( 'Skip Existing Items', 'vj-wp-import-export' ) );
                        ...
                        $this->set_log( "<strong>" . __( 'SKIPPED', 'vj-wp-import-export' ) . '</strong> : ' . __( 'Skip New Items', 'vj-wp-import-export' ) );
                ...
                if ( $this->backup_service !== false && $process_last_records === false && absint( $this->existing_item_id ) > 0 ) {
                        ...
                        if ( is_wp_error( $is_success ) ) {
                                $this->set_log( "<strong>" . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $is_success->get_error_message() );
                        }
                        unset( $is_success );
                }
                ...
                $this->set_log( $this->import_type . ' #' . $this->item_id . ' ' . __( 'Successfully Imported', 'vj-wp-import-export' ) );
        ...
        private function init_backup_services() {
                ...
                if ( $this->backup_service === false ) {
                        ...
                        if ( is_wp_error( $data ) ) {
                                $this->set_log( "<strong>" . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $data->get_error_message() );
                        }
                        unset( $data );
                }
        }
