...
class WPIE_Images {
        ...
        public static function wpie_get_image_from_local( $filename = "" ) {
                ...
                if ( !file_exists( $file ) ) {
                        ...
                        return new \WP_Error( 'imageDownloadError', '<strong>' . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $filename . " " . __( 'Image Not Exist', 'vj-wp-import-export' ) );
                }
                if ( !self::is_valid_image( $file ) ) {
                        return new \WP_Error( 'imageDownloadError', '<strong>' . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $filename . " " . __( 'File is not valid Image', 'vj-wp-import-export' ) );
                }
                ...
                return new \WP_Error( 'imageDownloadError', '<strong>' . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . __( 'Image Upload failed', 'vj-wp-import-export' ) );
        }
        ...
        public static function wpie_get_image_from_url( $file_url = "" ) {
                ...
                if ( empty( $file_url ) ) {
                        return new \WP_Error( 'wpie_import_error', __( 'File URL is empty', 'vj-wp-import-export' ) );
                }
                ...
                if ( \is_wp_error( $fileName ) ) {
                        return new \WP_Error( 'imageDownloadError', '<strong>' . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $fileName->get_error_message() );
                }
                ...
                if ( \is_wp_error( $file ) ) {
                        return new \WP_Error( 'imageDownloadError', '<strong>' . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $file->get_error_message() );
                }
                ...
                if ( \is_wp_error( $id ) ) {
                        ...
                        return new \WP_Error( 'imageDownloadError', '<strong>' . __( 'Warning', 'vj-wp-import-export' ) . '</strong> : ' . $id->get_error_message() );
                }
