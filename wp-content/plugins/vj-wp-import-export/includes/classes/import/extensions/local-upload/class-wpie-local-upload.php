...
class WPIE_Local_Upload extends \wpie\import\upload\WPIE_Upload {
        ...
        public function upload_local_file() {
                ...
                if ( ! preg_match( '%\W(zip|csv|xls|xlsx|xml|txt|json|ods|tar|gz)$%i', trim( basename( $fileName ) ) ) ) {
                        ...
                        return new \WP_Error( 'wpie_import_error', __( 'Uploaded file must be CSV, ZIP, XLS, XLSX, XML, TXT, JSON, ODS, TAR, GZ', 'vj-wp-import-export' ) );
                }
                ...
                if ( is_dir( WPIE_UPLOAD_TEMP_DIR ) && ($dir = opendir( WPIE_UPLOAD_TEMP_DIR )) ) {
                        ...
                        return new \WP_Error( 'wpie_import_error', __( 'Failed to open temp directory', 'vj-wp-import-export' ) );
                }
                unset( $maxFileAge );
                        ...
                        if ( $_FILES && file_exists( $_FILES[ 'local_file' ][ 'tmp_name' ] ) && is_uploaded_file( $_FILES[ 'local_file' ][ 'tmp_name' ] ) ) {
                                ...
                                if ( $out ) {
                                        ...
                                        if ( $in ) {
                                                ...
                                                return new \WP_Error( 'wpie_import_error', __( 'Failed to open input stream.', 'vj-wp-import-export' ) );
                                        }
                                        ...
                                        unset( $out );
                                        return new \WP_Error( 'wpie_import_error', __( 'Failed to open output stream.', 'vj-wp-import-export' ) );
                                }
                        } else {
                                return new \WP_Error( 'wpie_import_error', __( 'Failed to move uploaded file.', 'vj-wp-import-export' ) );
                        }
                } else {
