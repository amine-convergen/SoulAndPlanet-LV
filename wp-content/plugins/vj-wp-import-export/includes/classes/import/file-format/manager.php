...
class Manager {
        ...
        public function extract( $file = "", $to = "" ) {
                ...
                if ( ! is_readable( $file ) ) {
                        return new \WP_Error( 'wpie_import_error', __( 'Uploaded file is not readable', 'vj-wp-import-export' ) );
                }
                $this->file = $file;
                ...
                return new \WP_Error( 'wpie_import_error', __( "can't parse uploaded file", 'vj-wp-import-export' ) );
        }

        private function tar_extract() {

                if ( ! class_exists( '\PharData' ) ) {
                        return new \WP_Error( 'wpie_import_error', __( 'Class PharData Not Found', 'vj-wp-import-export' ) );
                }
        ...
        private function gz_extract() {
                ...
                if ( ! function_exists( "\gzopen" ) ) {
                        return new \WP_Error( 'wpie_import_error', __( 'Function GZOPEN Not Exist', 'vj-wp-import-export' ) );
                }

                $filename = substr( \basename( $this->file ), 0, -3 );

                if ( ! preg_match( '%\W(xml|zip|csv|xls|xlsx|xml|ods|txt|json|tar)$%i', trim( $filename ) ) ) {
                        return new \WP_Error( 'wpie_import_error', __( 'Uploaded file must be XML, CSV, ZIP, XLS, XLSX, XML, ODS, TXT, JSON, TAR', 'vj-wp-import-export' ) );
                }
                ...
                if ( $fp === false ) {
                        return new \WP_Error( 'wpie_import_error', __( "Can't Create file", 'vj-wp-import-export' ) );
                }

                $gfp = gzopen( $this->file, 'rb' );
                if ( $gfp === false ) {
                        fclose( $fp );
                        return new \WP_Error( 'wpie_import_error', __( "can't open GZ file", 'vj-wp-import-export' ) );
                }
