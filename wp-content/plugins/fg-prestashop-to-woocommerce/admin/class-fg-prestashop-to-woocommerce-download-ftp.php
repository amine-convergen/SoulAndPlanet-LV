...
if ( !class_exists('FG_PrestaShop_to_WooCommerce_Download_FTP', false) ) {
	 ...
	 */
	class FG_PrestaShop_to_WooCommerce_Download_FTP {
			...
			if ( !empty($this->plugin->ftp_options['hostname']) && !empty($this->plugin->ftp_options['username']) && !empty($this->plugin->ftp_options['password']) ) {
				...
				if ( $this->plugin->ftp_options['connection_type'] == 'sftp' ) {
					...
					if ( class_exists('WP_Filesystem_SSH2') ) {
						...
						$this->plugin->display_admin_error(__('FTP connection failed:', 'fg-prestashop-to-woocommerce') . ' ' . sprintf(__('(SFTP requires the <a href="%s" target="_blank">WP Filesystem SSH2</a> plugin)', 'fg-prestashop-to-woocommerce'), 'https://www.fredericgilles.net/wp-filesystem-ssh2/'));
						return false;
					}
				} else {
					// FTP and FTPS
				...
				if ( $this->ftp->connect() ) {
					if ( $this->ftp->is_dir($this->plugin->ftp_options['basedir']) ) {
						...
						$this->plugin->display_admin_error(__('FTP connection failed:', 'fg-prestashop-to-woocommerce') . ' ' . "Can't changedir to " . $this->plugin->ftp_options['basedir']);
					}
				} else {
					// Connection error
					$error_message = '';
					...
					$this->plugin->display_admin_error(__('FTP connection failed:', 'fg-prestashop-to-woocommerce') . ' ' . $error_message);
				}
			}
			return $result;
		}
