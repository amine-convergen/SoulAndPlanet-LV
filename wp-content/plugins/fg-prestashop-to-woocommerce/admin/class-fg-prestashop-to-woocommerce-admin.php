		...
		public function dispatch($action) {
			...
			elseif ( !empty($action) ) {
				switch($action) {
					...
					// Delete content
					case 'empty':
						if ( defined('WP_CLI') || check_admin_referer( 'empty', 'fgp2wc_nonce' ) ) { // Security check
...
SQL;
		...
		private function import_product_categories() {
			...
			foreach ( $categories as $category ) {
				...
				if ( !array_key_exists($category['id_category'], $this->imported_categories[$this->current_language]) ) { // Category not already imported
					$new_category_id = $this->import_product_category($category);
		...
		public function import_product_category($category) {
			...
			if ( !is_wp_error($new_term) ) {
				...
				add_term_meta($new_category_id, '_fgp2wc_old_product_category_id' . '-lang' . $this->current_language, $category['id_category'], true);
				...
				// Hook after inserting the category
				do_action('fgp2wc_post_insert_product_category', $new_category_id, $category);
			}
		...
		protected function get_specific_prices($product_id) {
			...
			if ( version_compare($this->prestashop_version, '1.4', '>=') ) {
				...
				$sql = "
					SELECT $product_attribute_column, p.price, p.from_quantity, p.reduction, $reduction_tax_field, p.reduction_type, p.`from`, p.`to`
		...
		private function get_cms_categories() {
			...
			if ( $this->table_exists($category_table) ) {
				...
				$sql = "
					SELECT DISTINCT c.id_${category_table} AS id_category, cl.name, cl.link_rewrite AS slug, cl.description, c.id_parent, $order
					FROM ${prefix}${category_table} c
					$extra_joins
		...
		protected function get_cms_articles($limit=1000) {
			...
			if ( version_compare($this->prestashop_version, '1.4', '<') ) {
				...
				$sql = "
					SELECT DISTINCT a.id_cms, l.meta_title, l.meta_description, l.meta_keywords, l.content, l.link_rewrite AS slug, a.id_cms_category AS id_category, a.position, a.active, $indexation_field, c.date_add AS date
					$extra_cols
					FROM ${prefix}cms a
					$extra_joins
		...
		private function get_all_product_categories() {
			...
			$sql = "
				SELECT DISTINCT c.id_category, c.date_add AS date, $position_field, c.id_parent, $root_category_field, cl.name, cl.description, cl.link_rewrite AS slug, cl.meta_description, cl.meta_keywords, cl.meta_title, $order
				FROM ${prefix}category c
				$extra_joins
		...
		private function get_products($limit=1000) {
			...
			if ( version_compare($this->prestashop_version, '1.5', '<') ) {
				...
				$sql = "
					SELECT p.id_product, p.id_supplier, p.id_manufacturer, $id_tax_rules_group, p.id_category_default, p.on_sale, p.quantity, p.price, p.wholesale_price, p.reference, p.ean13, p.supplier_reference, $location_field, $condition_field, $show_condition_field, $width_field, $height_field, $depth_field, p.weight, p.out_of_stock, p.active, $available_for_order_field, 'both' AS visibility, 0 AS is_virtual, p.date_add AS date, pl.name, pl.link_rewrite AS slug, pl.description, pl.description_short, pl.meta_description, pl.meta_keywords, pl.meta_title
					$reduction_fields
					FROM ${prefix}product p
